...
uint64_t service_stats_rx_kicks_sent = 0;
uint64_t service_stats_tx_kicks_sent = 0;
uint64_t service_stats_rx_full = 0;
uint64_t service_stats_rx_dequeued = 0;
uint64_t service_stats_rx_dequeued_local = 0;
uint64_t service_stats_select_called = 0;
uint64_t service_stats_select_returned = 0;
uint64_t service_stats_tx_buf_allocation_failure = 0;
uint64_t service_stats_send_failure = 0;
uint64_t service_stats_recv_failure = 0;
uint64_t service_stats_buffers_sent = 0;
uint64_t service_stats_buffers_allocated = 0;
uint64_t service_stats_cannot_allocate_cmd = 0;
uint64_t service_stats_rx_returned = 0;
uint64_t service_stats_accepted = 0;
uint64_t service_stats_flush_local = 0;
uint64_t service_stats_flush = 0;

pthread_t stats_thread;
uint8_t g_print_stats_loop = 1;
uint32_t g_client_ringset_idx = SERVICE_CONNECTION_POOL_SIZE;
service_update_cbk_t service_update_cbk = NULL;

void print_stats()
{
	service_log(SERVICE_LOG_INFO,"service_stats_receive_called %lu service_stats_send_called %lu \n\t\
                service_stats_rx_kicks_sent %lu service_stats_tx_kicks_sent %lu service_stats_cannot_allocate_cmd %lu  \n\t\
                service_stats_rx_full %lu service_stats_rx_dequeued %lu service_stats_rx_dequeued_local %lu \n\t\
                service_stats_select_called %lu service_stats_select_returned %lu service_stats_tx_buf_allocation_failure %lu \n\t\
                service_stats_send_failure %lu service_stats_recv_failure %lu service_stats_buffers_sent %lu service_stats_buffers_allocated %lu service_stats_rx_dequeued %lu\n",
                service_stats_receive_called,service_stats_send_called,service_stats_rx_kicks_sent,
                service_stats_tx_kicks_sent,service_stats_cannot_allocate_cmd,service_stats_rx_full,service_stats_rx_dequeued,
                service_stats_rx_dequeued_local,service_stats_select_called,service_stats_select_returned,service_stats_tx_buf_allocation_failure,
                service_stats_send_failure,service_stats_recv_failure,
                service_stats_buffers_sent,
                service_stats_buffers_allocated,
...
void print_rings_stats()
{
	...
	for(i = 0;i < SERVICE_CONNECTION_POOL_SIZE;i++) {
		if(local_socket_descriptors[i].socket) {
			service_log(SERVICE_LOG_INFO,"Connection #%d\n",i);
        		service_log(SERVICE_LOG_INFO,"rx ring count %d tx ring count %d",
				rte_ring_count(local_socket_descriptors[i].rx_ring),
				rte_ring_count(local_socket_descriptors[i].tx_ring));
			service_log(SERVICE_LOG_INFO,"local cache count %d",
				rte_ring_count(local_socket_descriptors[i].local_cache));
			service_log(SERVICE_LOG_INFO,"read_ready %d write_ready %d",
...
void sig_handler(int signum)
{
    ...
    service_log(SERVICE_LOG_INFO,"terminating on signal %d\n",signum);
    print_rings_stats();

    for(i = 0;i < SERVICE_CONNECTION_POOL_SIZE;i++) {
        if(local_socket_descriptors[i].socket) {
            service_close(local_socket_descriptors[i].socket->connection_idx);
        }
    }
    
    cmd = service_get_free_command_buf();
    if(cmd) {
       cmd->cmd = SERVICE_DISCONNECT_CLIENT;
...
int service_app_init(int argc,char **argv,char *app_unique_id)
{
    int i;
    char ringname[1024];
    rte_cpuset_t cpuset;
...
//    service_set_log_level(0);
    ...
    service_log(SERVICE_LOG_INFO,"EAL initialized\n");

    free_clients_ring = rte_ring_lookup(FREE_CLIENTS_RING);
    if(!free_clients_ring) {
        service_log(SERVICE_LOG_ERR,"cannot find ring %s %d\n",__FILE__,__LINE__);
        exit(0);
    }
    ...
    if(!free_connections_ring) {
        service_log(SERVICE_LOG_ERR,"cannot find free connections ring\n");
        return -1;
    }
    service_log(SERVICE_LOG_INFO,"free connections ring initialized\n");
    free_connections_pool = rte_mempool_lookup(FREE_CONNECTIONS_POOL_NAME);

    if(!free_connections_pool) {
        service_log(SERVICE_LOG_ERR,"cannot find free connections pool\n");
        return -1;
    }
    service_log(SERVICE_LOG_INFO,"free connections pool initialized\n");
    ...
    if(!tx_bufs_pool) {
        service_log(SERVICE_LOG_ERR,"cannot find tx bufs pool\n");
        return -1;
    }
    ...
    if(!rx_bufs_ring) {
        service_log(SERVICE_LOG_ERR,"cannot find rx bufs ring\n");
        return -1;
    }
...
//		unsigned char *p = (unsigned char *)&cpuset;
		...
		if(!rte_thread_set_affinity(&cpuset))
			break;
		service_log(SERVICE_LOG_ERR,"cannot set thread affinity for %d %s %d\n",cpu,__FILE__,__LINE__);
	}
    }
    return ((tx_bufs_pool == NULL)||(command_ring == NULL)||(free_command_pool == NULL));
}

int service_create_client(service_update_cbk_t update_cbk)
{
    int ringset_idx;
    service_cmd_t *cmd;
    char ringname[1024];
    ...
    cmd = service_get_free_command_buf();
    if(!cmd) {
       service_stats_cannot_allocate_cmd++;
       return -3;
    }

    cmd->cmd = SERVICE_CONNECT_CLIENT;
    ...
    service_update_cbk = update_cbk;
    return 0;
}

int service_read_updates(void)
{
	...
	switch(*p) {
		case SERVICE_NEW_IFACES:
		if(service_update_cbk) {
			cmd = 1;
			p++;
			service_update_cbk(cmd,p,rte_pktmbuf_data_len(mbuf) - 1);
		}
		break;
		case SERVICE_NEW_ADDRESSES:
		if(service_update_cbk) {
			cmd = 3;
			p++;
			service_update_cbk(cmd,p,rte_pktmbuf_data_len(mbuf) - 1);
		...
		break;
		case SERVICE_END_OF_RECORD:
		return 0;
	}
	return -1;
}

int service_open_select(void)
{
    int ringset_idx;
...
int service_set_socket_select(int sock,int select)
{
   service_cmd_t *cmd;
   ...
   cmd->cmd = SERVICE_SET_SOCKET_SELECT_COMMAND;
...
static inline void _service_remove_socket_from_cache(local_socket_descriptor_t *local_socket_descr,
							  int select)
{
	service_log(SERVICE_LOG_ERR,"%s %d %d\n",__FILE__,__LINE__, local_socket_descr->present_in_ready_cache);
...
static inline void service_remove_socket_from_cache(local_socket_descriptor_t *local_socket_descr)
{
	service_log(SERVICE_LOG_ERR,"%s %d %p\n",__FILE__,__LINE__, local_socket_descr);
	service_log(SERVICE_LOG_ERR,"%s %d %d\n",__FILE__,__LINE__, local_socket_descr->select);
	_service_remove_socket_from_cache(local_socket_descr,local_socket_descr->select);
}
#endif
void service_fdclear(int sock, struct service_fdset *fdset)
{
	fdset->interested_flags[sock] = 0;
	fdset->returned_flags[sock] = 0;
	service_log(SERVICE_LOG_ERR,"%s %d %d\n",__FILE__,__LINE__, sock);
#ifdef USE_LOCAL_READY_CACHE
	service_remove_socket_from_cache(&local_socket_descriptors[sock]);
#endif
}

static inline void service_reset_local_descriptor(int connection_idx)
{		
...
int service_open_socket(int family,int type,int parent)
{
	service_socket_t *service_socket;
	service_cmd_t *cmd;

	if(rte_ring_dequeue(free_connections_ring,(void **)&service_socket)) {
        	service_log(SERVICE_LOG_ERR,"%s %d\n",__FILE__,__LINE__);
        	return -1;
    	}
    	...
    	cmd->cmd = SERVICE_OPEN_SOCKET_COMMAND;
	...
	if(parent != -1)
		local_socket_descriptors[service_socket->connection_idx].select = parent;

    	return service_socket->connection_idx;
}

int service_bind(int sock, struct sockaddr *addr, __rte_unused int addrlen)
{
	service_cmd_t *cmd;
	...
	cmd->cmd = SERVICE_SOCKET_CONNECT_BIND_COMMAND;
	cmd->ringset_idx = sock;
...
int service_connect(int sock,struct sockaddr *addr, __rte_unused int addrlen)
{
	service_cmd_t *cmd;
	struct sockaddr_in *sockaddr_in = (struct sockaddr_in *)addr;
...
int service_listen_socket(int sock)
{
	service_cmd_t *cmd;
	...
	cmd->cmd = SERVICE_LISTEN_SOCKET_COMMAND;
	cmd->ringset_idx = sock;
	cmd->parent_idx = local_socket_descriptors[sock].select;
    	if(service_enqueue_command_buf(cmd)) {
       		service_free_command_buf(cmd);
...
void service_close(int sock)
{
    ...
    cmd->cmd = SERVICE_SOCKET_CLOSE_COMMAND;
    cmd->ringset_idx = sock;
    cmd->parent_idx = local_socket_descriptors[sock].select;
    if(service_enqueue_command_buf(cmd)) {
       service_free_command_buf(cmd);
    }
    service_flush_rx(sock);
#ifdef USE_LOCAL_READY_CACHE
    service_remove_socket_from_cache(&local_socket_descriptors[sock]);
#endif
}

static inline void service_notify_empty_tx_buffers(int sock)
{
    service_cmd_t *cmd;
    ...
    cmd->cmd = SERVICE_SOCKET_TX_POOL_EMPTY_COMMAND;
    cmd->ringset_idx = sock;
    if(service_enqueue_command_buf(cmd)) {
        service_free_command_buf(cmd);
    }
...
int service_get_socket_tx_space(int sock)
{
    int ring_space = service_socket_tx_space(sock);
	...
	rte_atomic16_set(&(local_socket_descriptors[sock & SOCKET_READY_MASK].socket->write_ready_to_app),0);
        service_notify_empty_tx_buffers(sock);
    }
    return rc;
}

int service_get_socket_tx_space_own_buffer(int sock)
{
    int ring_space = service_socket_tx_space(sock);
    int tx_space = rte_atomic32_read(&(local_socket_descriptors[sock & SOCKET_READY_MASK].socket->tx_space))/1448;
...
#define SERVICE_UPDATE_LOCAL_READY_CACHE(a,b,c) service_update_local_ready_cache(a,b,c,__func__,__LINE__)
static inline void service_update_local_ready_cache(int selector_idx,local_socket_descriptor_t *local_socket_descr,int bit,char *func,int line)
#else
#define SERVICE_UPDATE_LOCAL_READY_CACHE(a,b,c) service_update_local_ready_cache(a,b,c)
static inline void service_update_local_ready_cache(int selector_idx,local_socket_descriptor_t *local_socket_descr,int bit)
#endif
{
	if (selector_idx == -1) {
...
#endif
    service_stats_send_failure += (rc != 0);
    return rc;
}

inline int service_send_bulk(int sock,struct data_and_descriptor *bufs_and_desc,int *offsets,int *lengths,int buffer_count)
{
    ...
    service_stats_send_called++;
    service_stats_buffers_sent += buffer_count;
...
/* UDP or RAW */
int service_sendto(int sock,void *pdesc,int offset,int length, struct sockaddr *addr, __rte_unused int addrlen)
{
    int rc;
    struct rte_mbuf *mbuf = (struct rte_mbuf *)pdesc;
...
inline int service_sendto_bulk(int sock,struct data_and_descriptor *bufs_and_desc,int *offsets,int *lengths, struct sockaddr *addr,__rte_unused int addrlen,int buffer_count)
{
    int rc,idx,total_length = 0;
    struct rte_mbuf *mbufs[buffer_count];
...
static inline struct rte_mbuf *service_get_from_shadow(int sock)
{
	struct rte_mbuf *mbuf = NULL;
...
static inline void service_try_read_exact_amount(struct rte_mbuf *mbuf,int sock,int *total_len,int *first_segment_len)
{
...
/* TCP */
int service_receive(int sock,void **pbuffer,int *total_len,int *first_segment_len,void **pdesc)
{ 
    struct rte_mbuf *mbuf;

    service_stats_receive_called++;
    /* first try to look shadow. shadow pointer saved when last mbuf delievered partially */
    mbuf = service_get_from_shadow(sock);
	...
	service_try_read_exact_amount(mbuf,sock,&total_len2,first_segment_len);
	*pbuffer = rte_pktmbuf_mtod(mbuf,void *);
    	*pdesc = mbuf;
	if((total_len2 > 0)&&(total_len2 < *total_len)) { /* read less than user requested, try ring */
		struct rte_mbuf *mbuf2 = service_dequeue_rx_buf(sock);
		if(!mbuf2) { /* ring is empty */
			*total_len = total_len2;
			...
			service_try_read_exact_amount(mbuf2,sock,&total_len3,&first_segment_len_dummy);
...
read_from_ring:
    ...
    mbuf = service_dequeue_rx_buf(sock);
    if(mbuf) {
	if(*total_len > 0) { /* read exact */
		...
		*total_len = rte_pktmbuf_pkt_len(mbuf);
		*first_segment_len = rte_pktmbuf_data_len(mbuf);
		service_stats_rx_returned += mbuf->nb_segs;
	}
	*pbuffer = rte_pktmbuf_mtod(mbuf,void *);
    	*pdesc = mbuf;
	return 0;
...
#ifdef USE_LOCAL_READY_CACHE
    SERVICE_UPDATE_LOCAL_READY_CACHE(local_socket_descriptors[sock & SOCKET_READY_MASK].select,&local_socket_descriptors[sock & SOCKET_READY_MASK],SOCKET_READABLE_BIT);
#endif
    service_stats_recv_failure++;
    return -1;
}

/* UDP or RAW */
int service_receivefrom(int sock,void **buffer,int *len,struct sockaddr *addr,__rte_unused int *addrlen, void **pdesc)
{
    struct rte_mbuf *mbuf = service_dequeue_rx_buf(sock);
    struct sockaddr_in *in_addr = (struct sockaddr_in *)addr;
    service_stats_receive_called++;

    if(!mbuf) {
#ifdef USE_LOCAL_READY_CACHE
	SERVICE_UPDATE_LOCAL_READY_CACHE(local_socket_descriptors[sock].select,&local_socket_descriptors[sock],SOCKET_READABLE_BIT);
#endif
        service_stats_recv_failure++;
        return -1;
    }
...
void *service_get_buffer(int length,int owner_sock,void **pdesc)
{
    struct rte_mbuf *mbuf;
    mbuf = rte_pktmbuf_alloc(tx_bufs_pool);
    if(!mbuf) {
        service_notify_empty_tx_buffers(owner_sock);
        service_stats_tx_buf_allocation_failure++;
        return NULL;
    }
    service_stats_buffers_allocated++;
    *pdesc = mbuf;
...
int service_get_buffers_bulk(int length,int owner_sock,int count,struct data_and_descriptor *bufs_and_desc)
{
...
void service_release_rx_buffer(void *pdesc,int sock)
{
    struct rte_mbuf *mbuf = (struct rte_mbuf *)pdesc; 
...
int service_socket_kick(int sock)
{
    service_cmd_t *cmd;
    ...
    cmd->cmd = SERVICE_SOCKET_TX_KICK_COMMAND;
    cmd->ringset_idx = sock;
    if(service_enqueue_command_buf(cmd)) {
        service_free_command_buf(cmd);
    }
    else
        service_stats_tx_kicks_sent++;
    return 0;
}

int service_accept(int sock, struct sockaddr *addr, __rte_unused int *addrlen)
{
    service_cmd_t *cmd;
    service_socket_t *service_socket;
    ...
    if(rte_ring_dequeue(free_connections_ring,(void **)&service_socket)) {
        cmd->cmd = SERVICE_SOCKET_DECLINE_COMMAND;
	cmd->u.socket_decline.socket_descr = cmd->u.accepted_socket.socket_descr;
	if(service_enqueue_command_buf(cmd)) {
        	service_free_command_buf(cmd);
	        service_log(SERVICE_LOG_ERR,"CANNOT ENQUEUE SET_RING_COMMAND\n");
	        return -4;
    	}
	service_log(SERVICE_LOG_ERR,"NO FREE CONNECTIONS\n");
        return -3;
    } 
    ...
    local_socket_descriptors[service_socket->connection_idx].remote_ipaddr = in_addr->sin_addr.s_addr;
    local_socket_descriptors[service_socket->connection_idx].remote_port = in_addr->sin_port;
    local_socket_descriptors[service_socket->connection_idx].local_ipaddr =
	local_socket_descriptors[sock].local_ipaddr;
    local_socket_descriptors[service_socket->connection_idx].local_port =
	local_socket_descriptors[sock].local_port;
service_log(SERVICE_LOG_INFO,"%s %d acpt sock %p listen sock %d accpt idx %d rmt ip %x port %d local ip %x port %d\n",__func__,__LINE__,(void *)accepted_socket,sock,(int)service_socket->connection_idx,in_addr->sin_addr.s_addr,in_addr->sin_port,
local_socket_descriptors[service_socket->connection_idx].local_ipaddr,
local_socket_descriptors[service_socket->connection_idx].local_port);
    local_socket_descriptors[service_socket->connection_idx].socket = service_socket;
    ...
    service_stats_accepted++;
    return service_socket->connection_idx;
}

void service_getsockname(int sock,int is_local, struct sockaddr *addr, __rte_unused int *addrlen)
{
	struct sockaddr_in *in_addr = (struct sockaddr_in *)addr;
	in_addr->sin_family = AF_INET;
	if(is_local) {
...
static inline void service_free_common_notification_buf(service_cmd_t *cmd)
{
    rte_mempool_put(free_command_pool,(void *)cmd);
}

int service_is_connected(int sock)
{
    return local_socket_descriptors[sock].any_event_received;
}
#ifdef USE_LOCAL_READY_CACHE
void service_put_to_local_cache(int sock)
{
...
#endif
static inline int service_update_fdset(int idx, struct service_fdset *fdset)
{
	service_log(SERVICE_LOG_INFO,"%s %d %d \n",
...
int service_select(int selector,
			struct service_fdset *readfdset,
			struct service_fdset *writefdset,
			struct service_fdset *excfdset,
...
{ 
	...
	service_log(SERVICE_LOG_DEBUG,"%s %d sock %d mask %x readfdset mask %x writefdset mask %x\n",
...
restart_waiting:
    ...
    if (ret) {
	service_log(SERVICE_LOG_DEBUG,"%s %d ret %d\n",__func__,__LINE__,ret);
	return ret;
    }
    int ready_count = rte_ring_count(selectors[selector].ready_connections);
    if (unlikely(ready_count == 0)) {
	service_log(SERVICE_LOG_DEBUG,"%s %d exit_after_check %x timeout %x\n",
    ...
    service_log(SERVICE_LOG_DEBUG,"%s %d ready_count %d\n",__func__,__LINE__,ready_count);
    service_stats_select_returned++;
    for (i = 0;i < ready_count; i++) {
	    if(unlikely((ringset_idx_and_ready_mask[i] & SOCKET_READY_MASK) >= SERVICE_CONNECTION_POOL_SIZE)) {
        	service_log(SERVICE_LOG_ERR,"FATAL ERROR %s %d %d\n",__FILE__,__LINE__,ringset_idx_and_ready_mask[i] & SOCKET_READY_MASK);
	        exit(0);
	    } 
	    local_socket_descriptors[ringset_idx_and_ready_mask[i] & SOCKET_READY_MASK].any_event_received = 1;
#ifdef USE_LOCAL_READY_CACHE
	    service_log(SERVICE_LOG_DEBUG,"%s %d sock %d mask %x local_mask %x\n",
	    ...
	    service_put_to_local_cache(ringset_idx_and_ready_mask[i] & SOCKET_READY_MASK);
#endif
	    fd_updated = 0;
	    if (((ringset_idx_and_ready_mask[i] >> SOCKET_READY_SHIFT) & SOCKET_READABLE_BIT)&&(readfdset))
	    	fd_updated |= service_update_fdset(ringset_idx_and_ready_mask[i] & SOCKET_READY_MASK, readfdset);
	    if (((ringset_idx_and_ready_mask[i] >> SOCKET_READY_SHIFT) & SOCKET_CLOSED_BIT)&&(excfdset))
	    	fd_updated |= service_update_fdset(ringset_idx_and_ready_mask[i] & SOCKET_READY_MASK, excfdset);
	    else if (((ringset_idx_and_ready_mask[i] >> SOCKET_READY_SHIFT) & SOCKET_WRITABLE_BIT)&&(writefdset))
	    	fd_updated |= service_update_fdset(ringset_idx_and_ready_mask[i] & SOCKET_READY_MASK, writefdset);
#ifdef USE_LOCAL_READY_CACHE
	    if (fd_updated) {
		    service_log(SERVICE_LOG_DEBUG,"%s %d present_in_ready_cache %d\n",
			__func__,__LINE__,local_socket_descriptors[ringset_idx_and_ready_mask[i] & SOCKET_READY_MASK].present_in_ready_cache);

		    _service_remove_socket_from_cache(&local_socket_descriptors[ringset_idx_and_ready_mask[i] & SOCKET_READY_MASK], selector);
	/*	    if ((ringset_idx_and_ready_mask >> SOCKET_READY_SHIFT) & SOCKET_CLOSED_BIT) {
			service_close(ringset_idx_and_ready_mask & SOCKET_READY_MASK);
		    }*/
    	    }
#endif
    }
...
void *service_get_next_buffer_segment(void **pdesc,int *len)
{
   struct rte_mbuf *mbuf = (struct rte_mbuf *)*pdesc;
...
void *service_get_next_buffer_segment_and_detach_first(void **pdesc,int *len)
{
   struct rte_mbuf *mbuf = (struct rte_mbuf *)*pdesc;
...
static inline int service_route_command(int opcode,unsigned int ipaddr,unsigned int mask,unsigned int nexthop,short metric)
{
    service_cmd_t *cmd;
    cmd = service_get_free_command_buf();
    if(!cmd) {
...
int service_add_v4_route(unsigned int ipaddr,unsigned int mask,unsigned int nexthop,short metric)
{
    return service_route_command(SERVICE_ROUTE_ADD_COMMAND,ipaddr,mask,nexthop,metric);
}

int service_del_v4_route(unsigned int ipaddr,unsigned int mask,unsigned int nexthop)
{
    return service_route_command(SERVICE_ROUTE_DEL_COMMAND,ipaddr,mask,nexthop,0);
}

int service_setsockopt(int sock, int level, int optname,char *optval, unsigned int optlen)
{
    service_cmd_t *cmd;
    ...
    cmd->cmd = SERVICE_SETSOCKOPT_COMMAND;
    cmd->ringset_idx = sock;
    cmd->parent_idx = local_socket_descriptors[sock].select;
...
void service_set_buffer_data_len(void *buffer, int len)
{
    struct rte_mbuf *mbuf = (struct rte_mbuf *)buffer;
    rte_pktmbuf_data_len(mbuf) = len;
}

int service_get_buffer_data_len(void *buffer)
{
    struct rte_mbuf *mbuf = (struct rte_mbuf *)buffer;
    return rte_pktmbuf_data_len(mbuf);
}

void *service_get_data_ptr(void *desc)
{
	struct rte_mbuf *mbuf = (struct rte_mbuf *)desc;
	return rte_pktmbuf_mtod(mbuf, void *);
}

void service_update_rfc(void *desc, signed delta)
{
	struct rte_mbuf *mbuf = (struct rte_mbuf *)desc;
	rte_mbuf_refcnt_update(mbuf, delta);
}

int service_shutdown(int sock, int how)
{
	service_cmd_t *cmd;
    	cmd = service_get_free_command_buf();
    	if(!cmd) {
    	...
    	cmd->cmd = SERVICE_SOCKET_SHUTDOWN_COMMAND;
    	cmd->ringset_idx = sock;
    	cmd->parent_idx = local_socket_descriptors[sock].select;
...
void *service_create_ring(const char *name, int element_number)
{
...
void *service_ring_get(void *ring)
{
	void *obj = NULL;
...
void service_ring_free(void *ring, void *obj)
{
	rte_ring_enqueue(ring, obj);
}

void *service_mem_get(int size)
{
	return rte_zmalloc("", size, 0);
}

void service_mem_free(void *obj)
{
	rte_free(obj);
}
